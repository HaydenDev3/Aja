...
export default new (class EvalCommand implements Command {
    ...
    invoke = async (client: Client, message: Message, args: any) => {
        ...
        if (message.author.id === '622903645268344835') {
            var code = args;
            if ( !code )
                return await message.reply({ content: `> ${config.emojis.unicode.wrong} You need to provide me with some code first!` })
            code = args.join(' ');
            ...
            try {
                ...
                await message.reply({
                    content: `> ${config.emojis.unicode.correct} Successfully evaluated code!\n\`${codeType}\`\n\n${codeBlock("js", output)}`,
                });
            } catch (error: any) {
