...
export default new (class StickyMessagesCommand implements SlashCommand {
  ...
  constructor(
    private StickyMessagesService: StickyMessagesModule = Deps.get<StickyMessagesModule>(
  ...
  invoke = async (client: Client, interaction: ChatInputCommandInteraction) => {
    ...
    switch (subcommand) {
      ...
      case "edit": {
        ...
        const savedGuild =
          (await GuildSettings.findOne({ _id: interaction.guild?.id })) ||
          (await GuildSettings.create({ _id: interaction.guild?.id }));
        const stickyMessages = savedGuild.stickyMessages;

        if (!stickyMessages || stickyMessages.length === 0) {
          return interaction.followUp(
            `${config.emojis.unicode.wrong} There are no sticky messages in this guild.`
          );
        }

        const stickyMessage = stickyMessages.find(
          (m) => m.channelId === channel?.id
        );

        if (!stickyMessage) {
          return interaction.followUp(
            `${config.emojis.unicode.wrong} There is no sticky message in that channel.`
          );
        }

        try {
          await this.StickyMessagesService.updateStickyMessage(
            stickyMessage,
            { content },
            interaction.guild as any
          );
          return interaction.followUp(
            `${config.emojis.unicode.correct} Successfully updated the sticky message in ${channel}.`
          );
        } catch (error) {
          Log.fail(error);
          return interaction.followUp(
            `${config.emojis.unicode.wrong} An error occurred while updating the sticky message.`
          );
        }
      }
      ...
      case "delete": {
        ...
        if (!stickyMessage) {
          return interaction.followUp(
            `${config.emojis.unicode.correct} There is no sticky message in that channel.`
          );
        }

        try {
          await this.StickyMessagesService.deleteStickyMessage(
            interaction.channelId
          );
          return interaction.followUp(
            `${config.emojis.unicode.correct} Successfully deleted the sticky message in ${channel}.`
          );
        } catch (error) {
          Log.fail(error);
          return interaction.followUp(
            `${config.emojis.unicode.wrong} An error occurred while deleting the sticky message.`
          );
        }
      }
    }
