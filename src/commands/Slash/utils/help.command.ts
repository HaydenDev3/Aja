...
export default new (class HelpCommand implements SlashCommand {
  ...
  invoke = async (client: Client, interaction: ChatInputCommandInteraction) => {
      ...
      switch(guide) {
        case "commands": {
          await message.edit({
            embeds: [
              new EmbedBuilder()
                .setThumbnail('https://cdn.discordapp.com/emojis/1086196240208891944.webp?size=240&quality=lossless')
                .setTitle(`Here's a guide for Commands`)
                .setDescription(`> *Here's some commands that are available to you:*\n${commandList}`)
                .setColor(Colors.Blurple)
            ]
          })
          break;
        ...
        case "shields_guide": {
          await message.edit({
            embeds: [
              new EmbedBuilder()
                .setThumbnail('https://cdn.discordapp.com/emojis/1090080882091626577.webp?size=240&quality=lossless')
                .setTitle(`What are Shields?`)
                .setDescription(`> ${config.emojis.unicode.reply} Our **Shields** module allows your server to be protected from rule-breakers, you can enable the feature from using the ${config.emojis.unicode.slashCommand} \`/config\` command, however, if you haven't enrolled your guild do that by using the ${config.emojis.unicode.slashCommand} \`/register\` command.}`)
                .setColor(Colors.Blurple)
            ]
          })
          break;
    ...
    collector.on('end', async () => {
      await message.edit({
        content: "Your session has expired, please reuse the command to get this again",
        embeds: [],
        components: [],
      });
    });
