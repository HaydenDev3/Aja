...
export default class StickyMessagesModule extends RegisteringService {
  ...
  async fetch(guild: Guild) {
    const savedGuild =
      (await GuildSettings.findOne({ _id: guild.id })) ||
  ...
  async getStickyMessage(channel: TextChannel) {
    const savedGuild =
      (await GuildSettings.findOne({ _id: channel.guild.id })) ||
      (await GuildSettings.create({ _id: channel.guild.id }));
    var message = savedGuild.stickyMessages.find(
      (x: StickyMessage) => x.message.id === channel.id
    );
  ...
  async delete(message: StickyMessage) {
    const savedGuild =
      (await GuildSettings.findOne({ _id: message.message.guild.id })) ||
      (await GuildSettings.create({ _id: message.message.guild.id }));
    const index = savedGuild.stickyMessages.findIndex(
      (x: StickyMessage) => x.message.id === message.message.id
    );
    if (index !== -1) {
  ...
  async deleteStickyMessage(channelId: string) {
    ...
    if (savedGuild) {
      const index = savedGuild.stickyMessages.findIndex(
        (x: StickyMessage) => x.channelId === channelId
      );
      if (index !== -1) {
  ...
  async updateStickyMessage(
    ...
    options: { buttons?: IButton[]; content: string },
    guild: Guild
  ): Promise<void> {
    ...
    const index = savedStickyMessages.findIndex(
      (m) => m.channelId === stickyMessage.channelId
    );
    if (index >= 0) {
  ...
  async onInteractionCreate(interaction: ButtonInteraction) {
    ...
    const savedGuild =
      (await GuildSettings.findOne({ _id: interaction?.guildId })) ||
      (await GuildSettings.create({ _id: interaction.guildId }));
    const stickyMessage = savedGuild.stickyMessages.find(
      (msg) => msg.messageId === interaction.message.id
    );
