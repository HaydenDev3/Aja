...
export default class StarboardModule extends RegisteringService {
  public async init(message: Message) {
    ...
    if (
      starboard.enabled &&
      message.reactions.cache.get('⭐')!.count >= starboard.threshold
    ) {
      // Create new starboard message
      const starChannel = guild.channels.cache.get(
        starboard.channelId
      ) as TextChannel;
      if (!starChannel) {
        return;
      ...
      const existingMsg = await (await starChannel.messages.fetch())!.find(
